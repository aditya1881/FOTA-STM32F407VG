...
HAL_StatusTypeDef OtaMetadata_SetUpdatePending(uint32_t slot, uint32_t imageSize, uint32_t imageCrc, uint32_t newVersion)
{
  ...
  if (OtaMetadata_Load(&metadata) == 0U)
  {
    memcpy(&metadata, &gOtaMetadataDefault, sizeof(OtaMetadata_t));
  }

  metadata.metadataVersion++;
  metadata.updateInProgress = 1U;
  metadata.bootSlot = slot;
  metadata.bootAttemptCount = 0U;
  metadata.lastErrorCode = OTA_UPDATE_ERROR_NONE;

  if (slot == OTA_SLOT_A)
  {
    metadata.appASizeBytes = imageSize;
    metadata.appACrc32 = imageCrc;
    metadata.appAState = OTA_IMAGE_STATE_PENDING;
    metadata.appAVersion = newVersion;
  }
  else
  {
    metadata.appBSizeBytes = imageSize;
    metadata.appBCrc32 = imageCrc;
    metadata.appBState = OTA_IMAGE_STATE_PENDING;
    metadata.appBVersion = newVersion;
  }

  return OtaMetadata_Save(&metadata);
}
...
HAL_StatusTypeDef OtaMetadata_ConfirmActiveImage(void)
{
  ...
  slot = metadata.bootSlot;
  metadata.activeSlot = slot;
  metadata.confirmedSlot = slot;
  metadata.updateInProgress = 0U;
  metadata.bootAttemptCount = 0U;
  metadata.lastErrorCode = OTA_UPDATE_ERROR_NONE;
  metadata.metadataVersion++;
  ...
  if (slot == OTA_SLOT_A)
  {
    metadata.appAState = OTA_IMAGE_STATE_VALID;
  }
  else
  {
    metadata.appBState = OTA_IMAGE_STATE_VALID;
  }
